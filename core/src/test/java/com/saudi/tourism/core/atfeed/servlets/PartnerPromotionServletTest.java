...
public class PartnerPromotionServletTest {

	private PartnerPromotionServlet partnerPromotionServlet;
	...
	public void setUp(AemContext context) throws IOException {
		partnerPromotionServlet = new PartnerPromotionServlet();
...
//		when(context.request().getResourceResolver()).thenReturn(context.resourceResolver());
	...
	public void test(AemContext context)
		...
		partnerPromotionServlet.doGet(context.request(), context.response());
