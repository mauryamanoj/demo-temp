...
import com.saudi.tourism.core.services.article.v1.FetchArticlesHomeResponse;
...
class ArticlesHomeCFServletTest {

  @Mock
  private ArticlesCFService articlesCFService;

  @InjectMocks
  private ArticlesHomeCFServlet servlet = new ArticlesHomeCFServlet();
  ...
  void doGetShouldReturnArticlesIfLocaleIsProvided(final AemContext aemContext)
    ...
    when(articlesCFService.getArticlesHome(any(FetchArticlesRequest.class)))
      .thenReturn(FetchArticlesHomeResponse.builder().newArticles(Arrays.asList(articleSummary)).build());
    ...
    Assertions.assertEquals(
        "{\"code\":200,\"message\":\"success\",\"response\":{\"newArticles\":[{\"id\":\"Article1\",\"title\":\"Article1\"}]}}",
        response.getOutputAsString());
  }
