...
public class CardGridModelTest {
  ...
  @BeforeEach public void setUp(AemContext context) {
    context.load().json("/components/c97-card-grid/content.json", CONTENT_PATH);
  ...
  @Test public void testEventStatic(AemContext context) {
    String RESOURCE_PATH = CONTENT_PATH + "/jcr:content/root/responsivegrid/c97-card-grid";

    CardGridModel model =
        context.currentResource(RESOURCE_PATH).adaptTo(CardGridModel.class);

    assertEquals("/content/sauditourism/en/events/al/beethoven-night.html",
        model.getEvents().get(0).getEventLink().getUrlWithExtension());
    assertEquals("Beethoven Night", model.getEvents().get(0).getTitle());
