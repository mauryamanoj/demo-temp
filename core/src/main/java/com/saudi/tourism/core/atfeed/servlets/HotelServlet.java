...
@Component(service = Servlet.class,
    property = {Constants.SERVICE_DESCRIPTION + Constants.EQUAL + "Hotel Servlet",
    ServletResolverConstants.SLING_SERVLET_METHODS + Constants.EQUAL
      + HttpConstants.METHOD_GET,
    ServletResolverConstants.SLING_SERVLET_PATHS + Constants.EQUAL
      + "/bin/api/air/v1/hotel",
    ServletResolverConstants.SLING_SERVLET_EXTENSIONS + Constants.EQUAL + "csv"})
@Slf4j
public class HotelServlet extends AbstractATFeedServlet {
  ...
  public static final String HOTEL = "Accomodation";

  /** Static category of CARD_ITEMS . */
  ...
  protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
    ...
    try {
      ...
      if (sourcePath.contains(AIRConstants.DEFAULT_SEPARATOR)) {
            ...
            if (parentNode.hasNodes()) {
              ...
              while (items.hasNext()) {
                ...
                if (item.hasProperty(AIRConstants.TITLE)) {
                  entity.setId(ComponentUtils.generateId(CARD_ITEMS, item.getProperty(AIRConstants.TITLE)
                       .getString()));
        ...
        String csv = GeneralUtils.getHotelFeeds(request, entities, locale);
        getOutputStream(response, csv);
        LOGGER.debug("Success :{}, {}", "HotelServlet", response);
      }
    } catch (HTTPException h) {
      LOGGER.error("HTTP Exception Error",
          h.getLocalizedMessage());
    } catch (Exception e) {
      LOGGER.error("Error in servlet :{}, {}", "HotelServlet",
