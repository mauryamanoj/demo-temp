 ...
 */
@Component(service = AreaDetailsService.class,
    immediate = true)
@Slf4j
  ...
  private void setData(SlingHttpServletRequest request, TabDetailsModel detail, CardsRequestParams articleQueryParams,
    ...
    try {
      if (null != articleQueryParams) {
        CardsListModel details = cardsService.getFilteredCards(request, articleQueryParams, Constants.VERSION_V1);
        detail.setData(details.getData());
      }
      if (null != detail.getData() && StringUtils.isNotBlank(area)
          && StringUtils.isNotBlank(locale)) {
        detail.getData().forEach(det -> {
          CardsRequestParams cardsQueryParams = getCardsRequestParams(area, locale,
              type, det.getId());
          try {
            det.setOptions(cardsService.getFilteredCards(request, cardsQueryParams, Constants.VERSION_V1));
          } catch (RepositoryException e) {
            LOGGER.error("Error in forming article based cards list", e);
          }
        });
      }
    } catch (RepositoryException e) {
  ...
  private void setHotelsData(SlingHttpServletRequest request, TabDetailsModel detail,
    ...
    try {
      ...
      if (null != detail.getData() && StringUtils.isNotBlank(area)
          && StringUtils.isNotBlank(locale)) {
        detail.getData().forEach(det -> {
          HotelsRequestParams cardsQueryParams = getHotelsRequestParams(area, locale,
              det.getId());
          try {
            det.setOptions(hotelsService.getFilteredPackages(request, cardsQueryParams));
          } catch (RepositoryException e) {
            LOGGER.error("Error in forming article based cards list", e);
          }
        });
