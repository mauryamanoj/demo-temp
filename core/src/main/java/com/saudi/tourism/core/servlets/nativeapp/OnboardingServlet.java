...
@Component(service = Servlet.class, immediate = true, property = {Constants.SERVICE_DESCRIPTION
    + Constants.EQUAL + "Onboarding Servlet",
    ServletResolverConstants.SLING_SERVLET_METHODS + Constants.EQUAL
    + HttpConstants.METHOD_GET,
    ServletResolverConstants.SLING_SERVLET_PATHS + Constants.EQUAL
    + "/bin/api/v2/app/onboarding"})
public class OnboardingServlet extends SlingSafeMethodsServlet {
  ...
  protected void doGet(@NotNull SlingHttpServletRequest request,
    ...
    if (!StringUtils.isEmpty(resourcePath)) {
      OnboardingModel model = nativeAppHomepageService.getNativeOnboarding(request, resourcePath);
      if (model == null) {
        CommonUtils.writeNewJSONFormat(response, StatusEnum.NOT_FOUND.getValue(),
            "No Admin Onboarding  Config is found for this locale");
      } else {
        Gson gson = new Gson();
