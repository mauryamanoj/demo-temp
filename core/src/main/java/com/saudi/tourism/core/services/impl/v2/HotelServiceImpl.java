...
import static com.saudi.tourism.core.utils.Constants.MINUS_ONE;
import static com.saudi.tourism.core.utils.Constants.TYPE_PAGE_CONTENT;

/**
 * The type Package service implementation.
 */
@Component(service = HotelService.class, immediate = true)
@Slf4j
public class HotelServiceImpl implements HotelService {
   ...
   */
  private static final String[] FILTER_GRPS = {"hotelCity:3", "chain:4"};
  ...
  @Override
  public HotelsListModel getFilteredPackages(SlingHttpServletRequest request, HotelsRequestParams hotelsRequestParams) {

    HotelsListModel hotelsListModel = getAllPackages(request, hotelsRequestParams.getLocale());
    ...
    if (hotelsRequestParams.isAll()) {
      ...
      for (HotelDetail hotelItem : hotelsListModel.getData()) {
        hotelItem.setRelatedPackagesPaths(
            getRelatedPackagesPaths(request, hotelsRequestParams.getLocale(), hotelsRequestParams.isAllExpired(),
                hotelItem));
      }
  ...
  @Override
  public List<String> getRelatedPackagesPaths(SlingHttpServletRequest request, String language, Boolean isAllExpired,
    ...
    List<HotelDetail> filteredModel = filterPackagesOnParams(hotelsListModel, hotelsRequestParams).getData();
  ...
  private HotelsListModel getAllPackages(SlingHttpServletRequest request, String language) {
    String cacheKey = Constants.HOTELS_CACHE_KEY + Constants.VERSION_V2 + Constants.DASH + language;
    ...
    if (hotelsPath == null) {
      hotelsPath =
          Constants.ROOT_CONTENT_PATH + Constants.FORWARD_SLASH_CHARACTER + language + Constants.FORWARD_SLASH_CHARACTER
              + Constants.CONFIGS + Constants.FORWARD_SLASH_CHARACTER + "hotels";
    }
    Query query = queryBuilder.createQuery(PredicateGroup.create(getPredicateQueryMap(hotelsPath)),
   ...
   */
  private int getAllPackageDetails(final ResourceResolver resolver, final Iterator<Node> nodeIterator,
                                   final List<HotelDetail> hotelsList, int count) {
   ...
   */
  private HotelsListModel filterPackagesOnParams(HotelsListModel listModel, HotelsRequestParams hotelsRequestParams) {
    ...
    for (HotelDetail hotelItem : hotels) {
      ...
      if (!hotelItem.getPath().equals(hotelsRequestParams.getPath()) && matchFilters(hotelItem, hotelsRequestParams)) {
        total++;
        if (offset++ >= hotelsRequestParams.getOffset()) {
          count++;
          if (count <= hotelsRequestParams.getLimit() || hotelsRequestParams.getLimit()
              .equals(MINUS_ONE)) { // skip once limit is crossed
    ...
    AdminPageOption adminOptions = AdminUtil.getAdminOptions(language, StringUtils.EMPTY);
    boolean hideFilters =
        (StringUtils.isNotBlank(hotelsRequestParams.getArticleId()) && adminOptions.isDisableArticleHotelsFilter());
   ...
   * @return the boolean
   */
  private boolean matchFilters(HotelDetail hotelItem, HotelsRequestParams hotelsRequestParams) {

    boolean matchArea = Objects.isNull(hotelsRequestParams.getArea()) || (Objects.nonNull(hotelsRequestParams.getArea())
        && Objects.nonNull(hotelItem.getHotelCities()) && hotelItem.getHotelCities().stream().anyMatch(
          area -> hotelsRequestParams.getArea().stream().anyMatch(areaFilterItem -> areaFilterItem.contains(area))));
    boolean matchHotelChain =
        Objects.isNull(hotelsRequestParams.getHotelChain()) || (Objects.nonNull(hotelsRequestParams.getHotelChain())
            && Objects.nonNull(hotelItem.getHotelChains()) && hotelItem.getHotelChains().stream().anyMatch(
              hotelChain -> hotelsRequestParams.getHotelChain().stream()
                .anyMatch(hotelChainFilterItem -> hotelChainFilterItem.contains(hotelChain))));
    boolean matchArticle =
        Objects.isNull(hotelsRequestParams.getArticleId()) || (Objects.nonNull(hotelsRequestParams.getArticleId())
            && Objects.nonNull(hotelItem.getArticleIds()) && hotelItem.getArticleIds().stream()
            .anyMatch(hotelsRequestParams.getArticleId()::equalsIgnoreCase));
   ...
   * @return the string
   */
  private HotelsFilterModel getFiltersFromResult(SearchResult result, final TagManager tagManager) {
