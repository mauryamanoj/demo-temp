...
@Component(
    ...
    property = {
      ...
      ServletResolverConstants.SLING_SERVLET_PATHS
          + Constants.EQUAL
          + "/bin/api/mobile/v1/ssp-onboarding"
    })
public class SspOnboardingServlet extends SlingSafeMethodsServlet {

  /** error message. */
  public static final String NO_SSP_ONBOARDING_CONFIG_IS_FOUND_FOR_THIS_LOCALE =
      "No Ssp Onboarding  Config is found for this locale";
  ...
  protected void doGet(
    ...
    if (StringUtils.isBlank(resourcePath)) {
      LOGGER.error(Constants.DO_GET_MENTHOD, Constants.ERROR_MESSAGE_UNABLE_TO_ACCESS_SSP_CONFIG);
      MobileUtils.writeMobileV1Format(
          response,
          StatusEnum.BAD_REQUEST.getValue(),
          BusinessErrorCode.UNABLE_TO_FIND_RESOURCE.getCode(),
          new ResponseMessage(
                  MessageType.ERROR.getType(), Constants.ERROR_MESSAGE_UNABLE_TO_ACCESS_SSP_CONFIG)
              .toString());
      return;
    }

    try {
      final var model = nativeAppHomepageService.getSspOnboarding(request, resourcePath);
      ...
      if (Objects.isNull(model)) {
        ...
        new ResponseMessage(
                MessageType.ERROR.getType(), NO_SSP_ONBOARDING_CONFIG_IS_FOUND_FOR_THIS_LOCALE)
            .toString();
        return;
      }
      ...
      LOGGER.error("Error in getting module: {}", "/bin/api/mobile/v1/ssp-onboarding", e);
      MobileUtils.writeMobileV1Format(
          response,
  ...
  private enum BusinessErrorCode {
    ...
    /** No SSP Onboarding config found. */
    NO_SSP_ONBOARDING_FOUND(03),
    /** Unknown Error. */
    UNKNOWN_ERROR(99);
