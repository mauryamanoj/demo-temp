...
import static com.saudi.tourism.core.servlets.events.v1.EventsCFServlet.SERVLET_PATH;

/** Servlet to fetch events. */
@Component(
    service = Servlet.class,
    property = {
      Constants.SERVICE_DESCRIPTION + Constants.EQUAL + "Events Servlet",
...
@Slf4j
public class EventsCFServlet extends SlingAllMethodsServlet {

  /**
   * Possible paths for this servlet.
   */
  @SuppressWarnings("java:S1075")
  public static final String SERVLET_PATH = "/bin/api/v3/events";
  ...
  protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
    ...
    final var fetchEventsRequest =
        new Convert<>(request, FetchEventsRequest.class).getRequestData();

    try {

      if (StringUtils.isBlank(fetchEventsRequest.getLocale())) {
      ...
      final var model = eventsService.getFilteredEvents(fetchEventsRequest);
      CommonUtils.writeNewJSONFormat(
          response,
      ...
      LOGGER.error("Error to fetch events from CF", e);
      CommonUtils.writeNewJSONFormat(
