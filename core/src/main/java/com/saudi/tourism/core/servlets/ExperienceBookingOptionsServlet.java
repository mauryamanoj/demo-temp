...
/**
 * This is used to get experience booking options from the external site (
 * http://apilayer.net/api/live ) Sample URL :
 * http://localhost:4502/bin/api/v1/experiencebookingoptions . Sample URL :
 */
@Component(service = Servlet.class, property = {
    ...
    ServletResolverConstants.SLING_SERVLET_PATHS + Constants.EQUAL
        + "/bin/api/v1/experiencebookingoptions" })
@Slf4j
public class ExperienceBookingOptionsServlet extends SlingAllMethodsServlet {
  ...
  protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
    ...
    try {
      ...
      CommonUtils.writeJSON(response, StatusEnum.SUCCESS.getValue(),
          service.getExperienceBookingOptions(RestHelper.getParameters(request), experienceId));
