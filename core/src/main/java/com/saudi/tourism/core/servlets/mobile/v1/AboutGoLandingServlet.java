...
/** About Go Landing Servlet . */
@Component(
    service = Servlet.class,
    immediate = true,
    property = {
        Constants.SERVICE_DESCRIPTION + Constants.EQUAL + "About Go landing Page Servlet",
        ...
        ServletResolverConstants.SLING_SERVLET_PATHS
            + Constants.EQUAL
            + "/bin/api/mobile/v1/about-go-landing-page"
    })
public class AboutGoLandingServlet extends SlingSafeMethodsServlet {

  /** error message. */
  public static final String NO_ABOUT_GO_LANDING_CONFIG_IS_FOUND_FOR_THIS_LOCALE =
      "No About Go Landing Page Config Config is found for this locale";
  ...
  protected void doGet(
    ...
    try {
      final var model = nativeAppHomepageService.getAboutGoLandingConfig(request, resourcePath);

      if (Objects.isNull(model)) {
        MobileUtils.writeMobileV1Format(
            response,
            StatusEnum.NOT_FOUND.getValue(),
            BusinessErrorCode.NO_ABOUT_GO_LANDING_CONFIG_FOUND.getCode(),
            RestHelper.getObjectMapper().writeValueAsString(null));
        new ResponseMessage(
                MessageType.ERROR.getType(), NO_ABOUT_GO_LANDING_CONFIG_IS_FOUND_FOR_THIS_LOCALE)
            .toString();
        return;
      }
      ...
      LOGGER.error("Error in getting module: {}", "/bin/api/mobile/v1/about-go-landing-page", e);
      MobileUtils.writeMobileV1Format(
          response,
  ...
  /** Enum for business error codes used in the AboutGoLandingServlet. */
  private enum BusinessErrorCode {
    ...
    /** No About go config found. */
    NO_ABOUT_GO_LANDING_CONFIG_FOUND(03),
    /** Unknown Error. */
    UNKNOWN_ERROR(99);
