...
public class HighlightsHolidayCFAdapter implements HighlightsCFAdapter, ContentFragmentAwareModel {
  ...
  public boolean supports(Adaptable adaptable) {
    ...
    return StringUtils.equalsIgnoreCase("holiday", cf.getTemplate().getTitle());
  }
  ...
  public HighlightsCard adaptTo(Adaptable adaptable) {
    ...
    var fileReferenceLogo = getElementValue(cf, "logo", String.class);
    ...
    final var logo = new Image();

    logo.setFileReference(fileReferenceLogo);
    logo.setS7fileReference(s7logo);
