...
/** QR code tutorial Servlet . */
@Component(
    service = Servlet.class,
    immediate = true,
    property = {
      Constants.SERVICE_DESCRIPTION + Constants.EQUAL + "QR Code Tutorial Servlet",
      ...
      ServletResolverConstants.SLING_SERVLET_PATHS
          + Constants.EQUAL
          + "/bin/api/mobile/v1/qr-code-tutorial"
    })
public class QRCodeTutorialServlet extends SlingSafeMethodsServlet {

  /** error message. */
  public static final String NO_QR_CODE_CONFIG_CONFIG_IS_FOUND_FOR_THIS_LOCALE =
      "No QR Code tutorial  Config is found for this locale";
  ...
  protected void doGet(
    ...
    try {
      final var model = nativeAppHomepageService.getQrCodeTutorialConfig(request, resourcePath);

      if (Objects.isNull(model)) {
        MobileUtils.writeMobileV1Format(
            response,
            StatusEnum.NOT_FOUND.getValue(),
            BusinessErrorCode.NO_QR_CODE_TUTORIAL_CONFIG_FOUND.getCode(),
            RestHelper.getObjectMapper().writeValueAsString(null));
        new ResponseMessage(
                MessageType.ERROR.getType(), NO_QR_CODE_CONFIG_CONFIG_IS_FOUND_FOR_THIS_LOCALE)
            .toString();
        return;
      }
      ...
      LOGGER.error("Error in getting module: {}", "/bin/api/mobile/v1/qr-code-tutorial", e);
      MobileUtils.writeMobileV1Format(
          response,
  ...
  /** Enum for business error codes used in the QRCOdeTutorialServlet. */
  private enum BusinessErrorCode {
    ...
    /** No QR COde tutorial config found. */
    NO_QR_CODE_TUTORIAL_CONFIG_FOUND(03),
    /** Unknown Error. */
    UNKNOWN_ERROR(99);
