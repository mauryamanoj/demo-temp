...
@Component(service = Servlet.class, property = {
    ...
    ServletResolverConstants.SLING_SERVLET_PATHS + Constants.EQUAL + "/bin/api/v1/filters"})
@Slf4j public class EventsFiltersServlet extends SlingAllMethodsServlet {
  /**
   * The Event service.
   */
  ...
  @Override protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
    ...
    try {
      ...
      EventFilterModel filters = eventService.getEventFilters(eventsRequestParameters);
      CommonUtils.writeJSON(response, StatusEnum.SUCCESS.getValue(),
          new ObjectResponse<EventFilterModel>(MessageType.SUCCESS.getType(), filters));
