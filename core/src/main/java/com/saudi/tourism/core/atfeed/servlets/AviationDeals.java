...
@Component(service = Servlet.class,
    property = {Constants.SERVICE_DESCRIPTION + Constants.EQUAL + "Aviation Servlet",
    ServletResolverConstants.SLING_SERVLET_METHODS + Constants.EQUAL
      + HttpConstants.METHOD_GET,
    ServletResolverConstants.SLING_SERVLET_PATHS + Constants.EQUAL
      + "/bin/api/air/v1/aviation",
    ServletResolverConstants.SLING_SERVLET_EXTENSIONS + Constants.EQUAL + "csv"})
@Slf4j
public class AviationDeals extends AbstractATFeedServlet {
  ...
  /** Category of AVIATION deals. */
  public static final String AVIATION_CATEGORY = "aviation";
  /**
   * The Query builder.
   */
  @Reference
  ...
  protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
    ...
    try {
      ...
      if (sourcePath.contains(AIRConstants.DEFAULT_SEPARATOR)) {
          ...
          if (session.nodeExists(datalang)) {
            ...
            if (parentNode.hasNodes()) {
              ...
              while (items.hasNext()) {
                ...
                entity.setCategoryId(AVIATION_CATEGORY);
        ...
        String csv = GeneralUtils.getAviationFeedsCSV(request, entities, locale);
        getOutputStream(response, csv);
        LOGGER.debug("Success :{}, {}", "AviationServlet", response);
      }
    } catch (HTTPException h) {
      LOGGER.error("HTTP Except ion Error",
          h.getLocalizedMessage());
    } catch (Exception e) {
      LOGGER.error("Error in servlet :{}, {}", "AviationServlet",
