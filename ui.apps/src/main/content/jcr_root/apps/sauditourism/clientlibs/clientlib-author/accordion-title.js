...
(function (document, $, moment) {
  ...
  function extractValue($field) {
    ...
    if (value && $field.is(SELECTOR_DATE_FIELD)) {
      ...
      if (dateValue.isValid()) {
        var format = $field.attr(ATTR_DISPLAY_DATE_FORMAT) || DATE_DEFAULT_FORMAT;
  ...
  $(document).off(EVENT_DIALOG_LOAD).on(EVENT_DIALOG_LOAD, function () {
    var accordionItems = document.querySelectorAll(SELECTOR_ACCORDION_MULTIFIELD_ITEMS);
    ...
    Coral.commons.ready(accordionItems, function () {
      ...
      for (var i = accordionItems.length; i > 0; i--) {
        var accordionItem = accordionItems.item(i - 1),
         ...
         */
        var newTitleArr = $.grep($itemFieldsToProduceTitle.map(function (idx, titleField) {
          var id = titleField.id || titleField.name;
