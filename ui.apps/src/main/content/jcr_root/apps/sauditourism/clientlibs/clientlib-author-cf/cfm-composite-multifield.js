(function ($) {
    var CFM = window.Dam.CFM,
        CF_BASEPATH = '/content/dam/sauditourism',
    ...
    function extendRequestSave() {
        ...
        CFM.editor.Page.notifyNavigation = function (c, f) {
            var h = f && f.uiType || CFM.RequestManager.UI_MASK_IMMEDIATELY;
            CFM.RequestManager.finish(function () {
                c(!0)
            })
        };
    ...
    function isRTEField($field) {
        return $($field) && $($field).length > 0 && $($field).parent().find(".cfm-multieditor-richtext-container").size() > 0;
    }

    function setRTEField($field, value) {
        var $parent = $($field).parent();

        if ($parent.hasClass('cfm-multieditor')) {
            $parent.find('.cfm-multieditor-richtext-editor').empty().append(value);
        }
        $($field).val(value);
    }
    ...
    function fillMultifieldItems(mfItem) {
        ...
        try {
            ...
            return; // ends the function early if parsing fails
        }
    ...
    function canSkip(field) {
        ...
        switch (field.type) {
            case 'checkbox':
                ...
                break;
            case 'hidden':
                return field.name && field.name.includes("@");
                break;
            default:
                return false;
        }
